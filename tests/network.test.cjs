...
function setup(host, fetch) {
  let timeout;
  const window = {location:{hostname:host,origin:'https://'+host,href:'https://'+host+'/'},fetch};
...
test('regional calls and nested uploaded images use the same origin',async()=>{
  ...
  assert.equal(setup('likemestudio.github.io',async()=>{}).api.API,'https://fotohr-server-production.up.railway.app/api');
});
